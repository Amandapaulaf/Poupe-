...
async function getUserData(){
    ...
    .then(function(data){
        ...
        const incomes= transations.filter(({type}) => type === "INCOME")
        const expenses= transations.filter(({type}) => type === "EXPENSE")

        const totalIncome= incomes.reduce((accumulator, currentValue) => {return accumulator + currentValue.value}, 0)
        const totalExpense= expenses.reduce((accumulator, currentValue) => {return accumulator + currentValue.value}, 0)
        const total= totalIncome - totalExpense
        incomeElement.innerHTML= `<span> + R$ ${totalIncome} </span>`
